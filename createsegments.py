...
content_file = 'brad_pitt'
content_img_path = 'StyleTransfer/InputContentImages/' + content_file + '.jpg'
content_img = cv2.imread(content_img_path)

style_file = 'paintings1'
style_img_path = 'StyleTransfer/InputStyleImages/' + style_file + '.jpg'
style_img = cv2.resize(cv2.imread(style_img_path), (content_img.shape[1], content_img.shape[0]))

content_map_path = 'StyleTransfer/InputContentImageMasks/' + content_file + '_mask.jpg'
content_map = cv2.resize(cv2.imread(content_map_path), (content_img.shape[1], content_img.shape[0]))

style_map_path = 'StyleTransfer/InputStyleImageMasks/' + style_file + '_mask.jpg'
style_map = cv2.resize(cv2.imread(style_map_path), (content_img.shape[1], content_img.shape[0]))

style_output_dir = 'StyleTransfer/InputStyleSegments/'
content_output_dir = 'StyleTransfer/InputContentSegments/'
...
# BGR color scheme
map_colors = [[255, 0, 0], [0, 255, 0], [0, 0, 255], [0, 255, 255]]
    ...
    for x in xrange(0, cont_shape[0]):
        for y in xrange(0, cont_shape[1]):
            if np.abs(content_map[x][y][0] - color[0]) <= 10 and np.abs(content_map[x][y][1] - color[1]) <= 10 and np.abs(content_map[x][y][2] - color[2]) <= 10:
                ...
                new_content_img[x][y] = [104, 117, 124] # The mean used in style transfer
            if np.abs(style_map[x][y][0] - color[0]) <= 10 and np.abs(style_map[x][y][1] - color[1]) <= 10 and np.abs(style_map[x][y][2] - color[2]) <= 10:
                ...
                new_style_img[x][y] = [104, 117, 124] # The mean used in style transfer
